                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {posts.map((post: Post) => (
                        <Link
                            key={post.id}
                            href={`/blog/${post.slug}`}
                            className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-slate-100 hover:shadow-lg hover:shadow-slate-200/50 transition-all duration-300 ease-out hover:-translate-y-1"
                        >
                            ...
                            <div className="flex flex-1 flex-col p-5">
                                <div className="mb-3 flex items-center gap-2 text-xs font-medium text-slate-500">
                                    ...
                                    {post.categories?.nodes?.[0]?.name && (
                                        <>
                                            <span>•</span>
                                            <span className="text-indigo-500">{post.categories.nodes[0].name}</span>
                                        </>
                                    )}
                                </div>
                                <h3
                                    className="mb-3 text-lg font-bold leading-snug text-slate-900 group-hover:text-indigo-600 transition-colors"
                                    dangerouslySetInnerHTML={{ __html: post.title }}
                                />
                                <div
                                    className="line-clamp-2 text-sm text-slate-600 leading-relaxed"
